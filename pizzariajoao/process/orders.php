...
if ($method === "GET") {
    ...
    foreach ($pedidos as $pedido) {
        ...
        // resgatando a borda da pizza
        $tamanhosQuery = $conn->prepare("SELECT * FROM tamanhos WHERE id = :tamanho_id");

        $tamanhosQuery->bindParam(":tamanho_id", $pizzaData["tamanho_id"]);

        $tamanhosQuery->execute();

        $tamanhos = $tamanhosQuery->fetch(PDO::FETCH_ASSOC);

        $pizza["tamanhos"] = $tamanhos["tipo"];
